...
sub cpan_depends_ok_force_missing {
    ...
    return run_with_fake_modules { cpan_depends_ok($deps, $test_name) } %missing;
}
...
Test::CPAN - Test a distribution's interaction with CPAN before uploading.
...
Test::CPAN fools CPAN into thinking it has downloaded and unpacked your
...
time that C<Test::CPAN> is useful is when you depend on modules inside your
...
The following tests would be expected to pass:
  ...
  cpan_depends_ok_force_missing(
    [
        'My::Module', 'Test::CPAN', 'My::Module::Build'
    ],
    [
        'My::Module::Build'
    ],
...
for C<Test::CPAN> to work. It will call the "distdir" command on that script
